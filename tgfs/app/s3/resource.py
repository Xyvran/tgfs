...
from pys3.resource import Resource as _Resource
...
class Resource(_Resource):
    ...
    async def creation_date(self) -> str:
        return (await self.__fd()).created_at.isoformat()

    async def last_modified(self) -> str:
        return (await self.__fd()).get_latest_version().updated_at.isoformat()
